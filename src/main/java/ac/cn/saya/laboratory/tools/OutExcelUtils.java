...
public class OutExcelUtils {
     ...
     */
    public static void outExcelTemplateSimple(String[] keys, String[] titles, ArrayNode jsonObjectList, ServletOutputStream out) {
        try {
            ...
            for (int i = 1; i <= jsonObjectList.size(); i++) {
                ...
                JsonNode job = jsonObjectList.get(i - 1);
     ...
     * @param out            输出流
     */
    public static void outExcelTemplatePlus(String[] rootKeys, String[] childKeys, String[] titles, ArrayNode jsonObjectList, ServletOutputStream out) {
        try {
            // 第一步，创建一个workbook，对应一个Excel文件
            SXSSFWorkbook workbook = new SXSSFWorkbook();
            // 第二步，在webbook中添加一个sheet,对应Excel文件中的sheet
            ...
            int rowNum = 1;
            for (int i = 1; i <= jsonObjectList.size(); i++) {
                JsonNode rootLine = jsonObjectList.get(i - 1);
                // 获取详情数据
                JsonNode childJsonNode = rootLine.get(childKeys[0]);
                Iterator<JsonNode> elements = childJsonNode.elements();
                List<JsonNode> childLine = IteratorUtils.toList(elements);
                int childSize = 0;
                ...
                for (int j = 0; j < childSize; j++) {
                    ...
                    JsonNode child = childLine.get(j);
                    for (int k = 1; k < childKeys.length; k++) {
                        // 新建一列
                        SXSSFCell nextCell = nextRow.createCell(rootKeys.length + k - 1);
                        JsonNode property = child.get(childKeys[k]);
                        String val;
                        if (property != null && StringUtils.hasText(val = property.asText())) {
                            nextCell.setCellValue("null".equals(val)?"":val);
                        } else {
     ...
     */
    private static void writeCellDataForKey(SXSSFRow nextRow, String[] keys, JsonNode data, int startIndex) {
        for (int i = 0; i < keys.length; i++) {
            // 新建一列
            SXSSFCell nestCell = nextRow.createCell(startIndex + i);
            JsonNode property = data.get(keys[i]);
            String val;
            if (property != null && StringUtils.hasText(val = property.asText())) {
                nestCell.setCellValue("null".equals(val)?"":val);
            } else {
